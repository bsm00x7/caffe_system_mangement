...
class Data {
  ...
  static Future<List<Map<String, dynamic>>> getWeeklySales() async {
    ...
    final response = await _supabase
        ...
        .gte('purchase_date', startOfWeek.toIso8601String())
        .lte('purchase_date', now.toIso8601String())
        .order('purchase_date', ascending: true);
    
    return List<Map<String, dynamic>>.from(response);
  }

  // Get recent purchases (limited number)
  static Future<List<Map<String, dynamic>>> getRecentPurchases({int limit = 10}) async {
    final response = await _supabase
        .from('purchases')
        .select('*, users(name, email), items(item_name)')
        .order('purchase_date', ascending: false)
        .limit(limit);
  ...
  static Future<String> uploadProductImage(
    ...
    try {
      debugPrint('=== Data.uploadProductImage ===');
      ...
      debugPrint('Uploading to Supabase storage bucket: product-images');
      ...
      debugPrint('=== ERROR in Data.uploadProductImage ===');
      debugPrint('Error: $e');
